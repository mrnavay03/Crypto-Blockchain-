...
class Blockchain:
    ...
    def add_transaction(self, recipient, sender, signature, amount=1.0, is_receiving=False):
        """Adds the new transaction amount to the blockchain"""
        ...
        if Verification.verify_transaction(transaction, self.get_balance):
            ...
            if not is_receiving:
                for peer in self.__peer_nodes:
                    url = 'http://{}/broadcast_transaction'.format(peer)
                    try:
                        respond = requests.post(url, json={'sender': sender, 'recipient': recipient, 'amount': amount, 'signature': signature})
    ...
    def remove_remove_peer_node(self, node):
